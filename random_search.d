...
/+
    ...
    {// Sphere function - 100 dimensions
        ...
        foreach(i; 0 .. num)
        {
            auto res = random_search!real(v, &sphere, 3000UL, 20UL);
...
+/
    { // train a very small neural network on linear function
        ...
        true_nn.linear.relu.linear.softmax;
        ...
        auto res = random_search!float(sol, &loss_function, 1_000_000_000, 500);
...
/+
    { // train a very small neural network on dot product function
        ...
        auto sol = new Vector!float(nn.serialized_data.length, 0.0);
        auto res = random_search!float(sol, &loss_function, 1_000_000_000, 10_000);
